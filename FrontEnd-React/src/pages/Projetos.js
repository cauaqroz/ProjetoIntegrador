...
const Projetos = () => {
  ...
  useEffect(() => {
    if (selectedProjeto) {
      fetch(
        `http://localhost:2216/projetos/${selectedProjeto.id}/pedidosParticipacao`
      )
        .then((response) => response.json())
        .then((ids) => {
          Promise.all(
            ids.map((id) =>
              fetch(`http://localhost:2216/users/${id}`).then((res) =>
                res.json()
              )
            )
      ...
      if (
        ...
        Promise.all(
          selectedProjeto.approvedParticipants.map((id) =>
            fetch(`http://localhost:2216/users/${id}`).then((res) => res.json())
          )
  ...
  const handleAprovar = (userId) => {
    if (selectedProjeto && user) {
      fetch(
        `http://localhost:2216/projetos/${selectedProjeto.id}/aprovarUsuario`,
        {
          method: "POST",
          headers: {
  ...
  const handleNegar = (userId) => {
    if (selectedProjeto && user) {
      fetch(
        `http://localhost:2216/projetos/${selectedProjeto.id}/negarSolicitacao/${userId}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const enviarMensagem = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:2216/chat/${selectedProjeto.chatId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
